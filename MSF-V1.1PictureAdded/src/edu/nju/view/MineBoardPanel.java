...
public class MineBoardPanel extends JPanel implements Observer {
	...
	public void update(Observable o, Object arg) { 
		...
		if(updateMessage.getKey().equals("gameover")){
			BlockPO[][] bps= (BlockPO[][]) updateMessage.getValue();
			BlockVO bv;
			BlockPO bp;
			int Width = bps.length;
			int Height = bps[0].length;
			
            for(int i=0;i<Width;i++){
            	for(int j=0;j<Height;j++){        	  
            	    bp=bps[i][j];
            	    bv= bp.getDisplayBlock(GameState.OVER);
            	    jLabelButtons[bp.getY()][bp.getX()]
    					.setIcon(Images.getImageIconByState(bv.getState()));
            	    jLabelButtons[bp.getY()][bp.getX()].repaint();
                }
             }
		}
